...
export default async function WebsiteDetailPage({
  ...
  if (!website) notFound();
  const websiteAudits = audits.filter(
    (audit) => audit.websiteId === website.id,
  );
  const latest = latestAuditFor(website, audits);
  const findings = latest
    ? (await storeAdapter.getAuditReport(latest.id, user.id)).findings.filter(
  ...
  return (
    <AppShell
      ...
      subtitle={website.normalizedUrl}
    >
        ...
        <div className="card">
          ...
          <p className="muted">
            ...
            {website.nextScheduledRunAt
              ? timeAgo(website.nextScheduledRunAt)
              : "Not scheduled"}
          </p>
        </div>
      </div>
      ...
      <div className="grid cols-2">
        ...
        <div className="grid">
          <ScheduleForm
            ...
            frequency={website.scheduleFrequency}
            threshold={website.alertThreshold}
          />
          <EditWebsiteForm
            websiteId={website.id}
